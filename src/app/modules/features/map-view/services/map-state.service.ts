...
import { ICreationRules } from '../../admins-tool/map-creator/interfaces';
...
export class MapStateService implements IMapStateService, OnInit, OnDestroy {
  ...
  getBlankDistrict(): IDistrict {
    return {
      ...
      kingdomID: '0',
      position: { x: 0, y: 0 },
      resources: [this.getBlankResourceSource()],
      terrain: this.getBlankTerrainState(),
    };
  }

  getBlankResourceSource(): IResourceSource {
    return {
      id: '0', // Kingdom unique ID
  ...
  getBlankTerrainState(): ITerrainState {
    return {
  ...
  subscribeForMapData(): void {
    let sub$_1 = this.gameStateManager
      .getKingdomsManager()
      .watchForUpdates()
      .subscribe((items: IKingdom[]) => {
    ...
    let sub$_2 = this.gameStateManager
      .getDistrictsManager()
      .watchForUpdates()
      .subscribe((items: IDistrict[]) => {
  ...
  buildMap(): void {
    if (this.districtList.length > 0 && this.kingdomsList.length >= 0) {
      ...
      this.map = this.generateEmptyMap(
        lastDistrict.position.x + 1,
        lastDistrict.position.y + 1
      );
      ...
      this.logger.debug(
        'buildMap() called but not all list are fetched yet...',
        this.logPrefix
      );
    }
  }
