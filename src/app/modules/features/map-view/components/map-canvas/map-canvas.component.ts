...
export class MapCanvasComponent implements OnInit, OnDestroy {
  ...
  private readonly scaleMinValue = 0.1;
  ...
  ngOnInit(): void {
    if (this.mapCanvas) {
      this.mapSubscription$ = this.mapStateService
        .subscribeMap()
        .subscribe((map) => {
          this.gameMap = map;
          this.loggerService.debug(
            `Painting map after new data`,
            this.loggerPrefix
          );
          this.repaintCanvas();
        });
      ...
      this.clickRegionCanvas;
      this.mapCanvas.nativeElement.onmousemove = (mouseEvent) =>
        this.onMove(mouseEvent);
      this.mapCanvas.nativeElement.onmousedown = (mouseEvent) =>
        this.onClick(mouseEvent);
      this.mapCanvas.nativeElement.onmouseup = (mouseEvent) =>
        this.onRelease(mouseEvent);
      this.mapCanvas.nativeElement.onwheel = (wheelEvent) =>
        this.onScroll(wheelEvent);
      this.loadImages();
    }
  }
  ...
  private async moveMap() {
    ...
    do {
      ...
      leftPadding =
        this.mousePositionX < movingPaddingWidth && this.mousePositionX > 0;
      rightPadding =
        this.mousePositionX > this.canvasWidth - movingPaddingWidth &&
        this.mousePositionX < this.canvasWidth;
      topPadding =
        this.mousePositionY < movingPaddingHeight && this.mousePositionY > 0;
      bottomPadding =
        this.mousePositionY > this.canvasHeight - movingPaddingHeight &&
        this.mousePositionY < this.canvasHeight;

      //2nd check if map should be moved
      let x_speed = this.movingSpeedRatio * this.canvasWidth;
      let x_direction =
        leftPadding || rightPadding ? (leftPadding ? -x_speed : x_speed) : 0;
      let y_direction =
        topPadding || bottomPadding ? (topPadding ? -x_speed : x_speed) : 0;

      if (leftPadding || rightPadding || topPadding || bottomPadding) {
        this.mapOffsetX -= x_direction;
        this.mapOffsetY -= y_direction;
        this.repaintCanvas();
        await this.wait(50);
    ...
    for (let path of imageToLoad) {
      ...
      image.onload = () => {
        ...
        this.loggerService.info(
          `Loaded ${imageToLoad.length - loadingImages} out of ${
            imageToLoad.length
          }`,
          this.loggerPrefix
        );
  ...
  private paintMap() {
    if (this.ctx && this.gameMap[0]) {
      ...
      let cityImg =
        this.imageMap.get(this.imagesPath.progress.cities.lv1) || new Image();
      ...
      for (const row of this.gameMap) {
        for (const mapTile of row) {
          ...
          let position_y =
            y * hex_height + (isOdd ? 0 : hex_height * 0.5) + this.mapOffsetY;
          ...
          switch (mapTile.district.terrain.type) {
            case TerrainType.FIELDS:
              terrainImg =
                this.imageMap.get(this.imagesPath.terrain.fields) ||
                new Image();
              break;
            case TerrainType.FOREST:
              terrainImg =
                this.imageMap.get(this.imagesPath.terrain.forest) ||
                new Image();
              break;
            case TerrainType.SEA:
              terrainImg =
                this.imageMap.get(this.imagesPath.terrain.sea) || new Image();
              break;
          }
  ...
  private paintRegionMap(context: CanvasRenderingContext2D) {
    ...
    for (const row of this.gameMap) {
      for (const mapTile of row) {
        ...
        context.beginPath();
        context.arc(
          position_x + 0.5 * hex_width,
          position_y + 0.5 * hex_width,
          0.55 * hex_height,
          0,
          2 * Math.PI,
          false
  ...
  private onRelease(mouseEv: MouseEvent) {
    ...
    if (mouseEv.button === 0 && this.ctx && this.gameMap[0]) {
  ...
  private onMove(mouseEv: MouseEvent) {
    ...
    if (!this.mouseDragged) {
      ...
      const mapTile = this.regionMap.get(regionKey);
      if (mapTile && this.ctx) {
        this.repaintHex(this.hoverRegion);
  ...
  private onScroll(wheelEv: WheelEvent) {
    let newScale =
      this.scaleFactor +
      (wheelEv.deltaY < 0 ? this.scaleJumps : -this.scaleJumps);
  ...
  private getRegionKey(clickX: number, clickY: number): string {
    if (this.clickRegionContext) {
      const clickData = this.clickRegionContext.getImageData(
        clickX,
        clickY,
        1,
        1
    ...
    if (region) {
      ...
      if (region.kingdom)
        this.paintInGrid(
          region.position.x,
          region.position.y,
          this.imagesPath.progress.cities.lv1
        );
    }
  }
  ...
  private paintHex(
    ...
    context.drawImage(
      image,
      position_x + this.mapOffsetX,
      position_y + this.mapOffsetY,
      hex_width,
      hex_height
    );
  }
   ...
   */
  private paintHexRegion(
    x: number,
    y: number,
    size: number,
    mouseMap: CanvasRenderingContext2D,
    hexUniqueRGB: string
  ) {
    let hex_width = size;
    let hex_height = size * this.hexRatio;
    ...
    mouseMap.beginPath();
    mouseMap.arc(
      position_x + 0.5 * hex_width + this.mapOffsetX,
      position_y + 0.5 * hex_width + this.mapOffsetY,
      0.55 * hex_height,
      0,
      2 * Math.PI,
      false
