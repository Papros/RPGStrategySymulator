...
import { IDistrict, IKingdom, IMap } from '@app/services/storage/interfaces';
...
export class MapService extends GameDataService<IMap> {
  ...
  fetchItems(): Observable<IMap[]> {
    ...
    request.pipe(take(1)).subscribe((data: IMap[]) => {
      this.items = data;
      this.logger.info('Map API response.', this.loggerPrefix);
    });
    return request;
  }
}
